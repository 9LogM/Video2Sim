...
"
    ...
    echo "--- Found existing Wonder3D+ models at ${WONDER3D_DIR}. ---"
fi
...
if [ ! -f "${LAMA_CHECK_FILE}" ]; then
    echo "--- LaMa model not found. Downloading into ${LAMA_DIR}... ---"
    mkdir -p "${LAMA_DIR}"
    ...
    echo "--- Found existing LaMa model at ${LAMA_CHECK_FILE}. ---"
fi
...
if [ ! -f "${OMNIDATA_FILE}" ]; then
    echo "--- Omnidata model not found. Downloading to ${OMNIDATA_FILE}... ---"
    ...
    echo "--- Found existing Omnidata model at ${OMNIDATA_FILE}. ---"
fi
...
############################################
# Symlink cache to holoscene directory
############################################
...
############################################
echo "--- Running Stage 0: Generating priors with Marigold... ---"
